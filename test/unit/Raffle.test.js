    ...
    : describe("Raffle Unit Tests", function () {
          ...
          describe("checkUpkeep", function () {
              ...
              it("returns false if enough time hasn't passed", async () => {
                  await raffle.enterRaffle({ value: raffleEntranceFee })
                  await network.provider.send("evm_increaseTime", [ethers.toNumber(interval) - 5]) // use a higher number here if this test fails
                  await network.provider.request({ method: "evm_mine", params: [] })
                  const { upkeepNeeded } = await raffle.checkUpkeep("0x") // upkeepNeeded = (timePassed && isOpen && hasBalance && hasPlayers)
                  assert(!upkeepNeeded)
              })
                          ...
                          try {
                              ...
                              assert.equal(
                                  ...
                                  (
                                      startingBalance +
                                      raffleEntranceFee +
                                      raffleEntranceFee * ethers.toBigInt(additionalEntrances)
                                  ).toString(),
                              )
